...
**PTY runtime:** `crates/openab-pty/` — remote sandboxed terminal sessions (**internal, unpublished**: no image, no chart, no user docs — see the Gate A revision on the tracking issue). Design of record: [`docs/adr/openab-pty-runtime.md`](docs/adr/openab-pty-runtime.md). Two invariants that must not be quietly relaxed: the **admin plane is remote-only and the credential is its whole boundary** — the claim is that **a managed session cannot authenticate to, or successfully invoke, admin operations**, *not* that the admin plane is unreachable by construction (no UDS, no second loopback listener, no in-container CLI, so no code path treats being inside the container as authorization; but `/admin/*` shares the attach listener, so the one listener *is* reachable from a managed session, which is why the adversary test asserts a `401` rather than a refused connection, and why the ADR states the residual local-DoS and in-session-guessing risks), and **teardown is best-effort** (Tier 1 is the only kill domain implemented and must be labelled as such wherever surfaced; `kill_domain_tier = "tier2-required"` is refused at startup because Tier 2 is not implemented). Linux-only mechanisms (`PR_SET_DUMPABLE`) are `#[cfg]`-gated and inert on macOS; the adversary tests in `tests/adversary_linux.rs` only run on Linux.
