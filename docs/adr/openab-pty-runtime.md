...
> **Granularity note — target contracts**: the MUST inventories in this section and in Session lifecycle are **target contracts**: they bind the Phase 1 acceptance tests, and implementation evidence may revise them via PRs against this ADR (each revision recorded, never silent). Phase 1 was to include extracting them into **`docs/pty-security-contract.md`** as given/when/assert acceptance criteria — after which this ADR would record *decisions and rationale* and the contract document would own the *testable clauses*. **That extraction is deferred to Gate B, recorded as such in the Phase 1 list**, because the crate is unpublished; until then the clauses live here and the tests bind to them directly. This resolves the ADR-to-spec granularity drift deliberately rather than letting the two genres blur.
...
- **Token control plane** (MVP model; an identity layer remains explicitly out of scope per `identity-trust-none.md`):
  ...
  - **Why remote-only, and what it eliminates**: locality was never authentication (the shell child may share the container and the UID), so an in-container admin socket would have required proving the caller is not inside a managed session -- which in turn required a kernel-authoritative membership primitive. Removing the socket removes that requirement: there is no path on which *being inside* authorizes anything, so no membership check is owed. It does **not** make the admin plane unreachable from a session child -- see the correction below. It also eliminates the in-container plaintext-credential problem entirely -- with no helper on the presentation path there is no splice-only CLI, no argv/environment/tempfile exposure, and no "dumpable before file-descriptor possession" timing window to satisfy (a constraint that is in any case **unsatisfiable** for a credential-bearing stdin inherited at `exec`, since the descriptor and its buffered bytes exist before the new program can call `prctl`). This is elimination rather than hardening, matching the keyless token model's approach.
  - **A managed session cannot authenticate to, or successfully invoke, admin operations (MUST)** -- *corrected 2026-08-16, during Phase 1 implementation.* The original wording ("admin-plane-unreachable, by construction: no admin operation is reachable from inside a managed session") was wrong, and the implementation's own adversary test is the evidence: the admin routes are served on the **same listener** as attach (default `0.0.0.0:8090`), and a managed session shares the runtime's network namespace, so a session child *can* connect and does -- it receives `401 Unauthorized`. Reachable-but-unauthenticated is a materially weaker claim than unreachable, with different residual risks, and it is the claim this ADR makes. What is true, and what this clause now requires: **no admin-only endpoint exists** (no UDS, no second loopback listener, no in-container CLI), so **no code path treats being inside the container as authorization**; every admin operation is gated by the remote credential, which exists in the container only as a non-reversible hash, so a session child has nothing to authenticate with, and tokens are never written to a PTY master. **Stated residual risks of that reachability**, which the stronger claim used to hide: (a) **local DoS against the admin plane from inside a session** -- a session child can spend request-handling capacity on rejected admin requests. Bounded, not prevented, by three things: the concurrency cap on in-flight verifications, the bounded request body, and the failure throttle, which is **per source**, short-circuits repeated rejected attempts (a request with no usable credential is refused before any comparison), and never extends an active block -- so a hostile source can neither delay another source's authentication nor hold its own block open. The throttle deliberately does not refuse a *correct* credential: doing so would buy nothing against the search space below and would cost the operator exactly the recovery path they need while something is hammering the endpoint; (b) **in-session credential guessing** -- infeasible rather than prevented: a 256-bit CSPRNG credential, stored only as a SHA-256 verifier, compared in constant time, and rate-limited per source. Rationale for the requirement is unchanged: running `session create/renew` *inside* a managed PTY would print the one-time token into the PTY byte stream, landing it in the scrollback ring and every attach/replay client -- a server-side exfiltration channel no client-storage rule can close. Tokens are returned only to the external control client and are never written to any PTY master. Phase 1 test: no admin-only socket (TCP or UNIX) appears while the runtime runs, and an admin request issued from inside a managed session is refused.
  ...
  - **Phase 1 adversary tests**: `ptrace` and `/proc/<pid>/{mem,fd}` probes against the runtime are **denied (`EPERM` or `EACCES` -- measured, `/proc/<pid>/mem` denial is `EACCES`)**, with a positive control asserting the same probes *succeed* at dumpable=1 so a pass cannot be an artifact of Yama `ptrace_scope`; dumpability is asserted to remain 0 after startup (regression guard -- a dependency calling `prctl(PR_SET_DUMPABLE, 1)` must fail the test); `prctl` failure refuses service; no admin-only socket exists and an admin request from inside a managed session is refused; and teardown is exercised against a `setsid` escapee, a double-fork with an early-exiting intermediate, and a `SIGTERM`-trapping child, asserting the tier's documented semantics (Tier 1: survivors detected and audited; Tier 2: zero survivors).
...
- portable-pty spawner with `setpgid`, escalating kill, the teardown order above, and **Tier 1 of the kill domain** (pgid + subreaper/pidfd, best effort, labelled as such wherever surfaced). **Tier 2 is not implemented in Phase 1** -- *recorded 2026-08-16*: its guarantee requires the child's pid to enter the cgroup before `execv`, and `portable-pty` owns its own pre-exec closure, so no Tier 2 code path was reachable from a real session. It remains the design of record above; `kill_domain_tier = "tier2-required"` is refused at startup rather than silently downgraded, and reinstating Tier 2 requires the pre-`execv` cgroup-join spawn primitive first
...
- ~~**Extract the security/lifecycle MUST inventories into `docs/pty-security-contract.md`**~~ -- **DEFERRED 2026-08-16, recorded here rather than dropped silently** (this ADR requires each revision be recorded, never silent). The Gate A revision on the tracking issue made the crate internal and unpublished: no image, no chart, no user docs. A second document restating the MUST clauses would have to be kept in step with an ADR that is still moving, for no reader this ADR does not already serve. The clauses stay here and remain bound by the Phase 1 adversary/e2e tests named below; the extraction returns with Gate B, when publishing gives the contract document an audience
- **Adversary tests** (see the Security model for the full list): `ptrace` and `/proc/<pid>/{mem,fd}` probes against the runtime are **denied (`EPERM` or `EACCES`)** with a **positive control** proving the same probes succeed at dumpable=1, so a pass cannot be an artifact of Yama `ptrace_scope`; a dumpable regression guard; `prctl` failure refuses service; no admin-only socket exists, and an admin request from inside a managed session arrives and is refused `401`; and a teardown suite (`setsid` escapee, double-fork with early-exiting intermediate, `SIGTERM`-trapping child) asserting Tier 1's documented semantics -- survivors it can see are detected, audited, and counted, and the `setsid` escapee is measured as surviving undiscoverably
