...
- **Token control plane** (MVP model; an identity layer remains explicitly out of scope per `identity-trust-none.md`):
  ...
  - **No privileged admin path from inside a managed session (MUST)** -- *corrected 2026-08-16, during Phase 1 implementation.* The original wording ("admin-plane-unreachable, by construction: no admin operation is reachable from inside a managed session") was wrong, and the implementation's own adversary test is the evidence: the admin routes are served on the **same listener** as attach, and a managed session shares the runtime's network namespace, so a session child *can* connect and does -- it receives `401 Unauthorized`. What is true, and what this clause now requires: **no admin-only endpoint exists** (no UDS, no second loopback listener, no in-container CLI), so **no code path treats being inside the container as authorization**, and every admin operation is gated by the remote credential, which exists only on the operator's side and never enters the container. Rationale for the requirement is unchanged: running `session create/renew` *inside* a managed PTY would print the one-time token into the PTY byte stream, landing it in the scrollback ring and every attach/replay client -- a server-side exfiltration channel no client-storage rule can close. Tokens are returned only to the external control client and are never written to any PTY master. Phase 1 test: no admin-only socket (TCP or UNIX) appears while the runtime runs, and an admin request issued from inside a managed session is refused.
