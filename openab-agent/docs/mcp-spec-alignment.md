...
| 492 | `execution.taskSupport` values: `"forbidden"` (default), `"optional"`, `"required"` | (field) | ⚠️ | **partial**: `task_support` value now surfaced to the LLM via `tool_summary` (`src/mcp/meta_tool.rs:295`) in both `list_tools`/`describe_tool`, so the LLM can see a tool's declared mode. We still do not *enforce* it — a tool marked `taskSupport:"required"` is invoked normally (we don't implement tasks). Refusal guard deferred (needs tools-cache to avoid an extra `tools/list` round-trip per call) — Row 289 / Section 7 |
...
- [ ] **Row 492 — honour `execution.taskSupport`** — surfacing half DONE (the value now reaches the LLM via `tool_summary`, `src/mcp/meta_tool.rs:295`). Remaining: a hard refusal guard on `call` actions for tools declaring `"required"` (we don't implement tasks).
  - **Eval**: openab-agent only · drop-in (~20 LOC guard) · **fit: in-scope but deferred**. The guard needs a per-server tools-cache to inspect `task_support` without an extra `tools/list` round-trip per call (and to avoid breaking `call_null_arguments_passes_validation_and_reaches_connect`). Land bundled with the Row 503 tools-cache / Section 7 Improvement #1.
