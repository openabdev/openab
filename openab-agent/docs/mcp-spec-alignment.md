...
- [ ] **(Whole `roots` capability)**: implementing client-side `roots` means committing to a workspace/path model (which directories does an openab-agent session expose? the ACP `cwd`? a configured allow-list?), declaring `capabilities.roots.listChanged`, plumbing `notify_roots_list_changed` on workspace changes, and enforcing #371–#374 (permission/validation/access-control/accessibility monitoring). The natural source of truth is the ACP session's working directory plus any host-supplied allow-list.
  - **Eval**: openab-agent layer (rmcp has `Root`/`ListRootsResult`/`notify_roots_list_changed` primitives, no convenience layer) · architectural commitment (~400-700 LOC: `list_roots` on the `OpenabClientHandler` keystone + workspace model + path validator + change-watcher + ACP cwd wiring) · **fit: in-scope**.
  - **DECISION (Brett 2026-06-05): in-scope.** Stable (non-experimental) standard capability and the most-likely-needed client cap for filesystem-scoped servers. Root source is non-interactive (ACP session `cwd` + optional config allow-list), so no consent-UI dependency (unlike URL elicitation / HITL). Implement on the keystone: declare `capabilities.roots`(+`.listChanged`), derive roots from ACP `cwd`/allow-list, override `list_roots` to return them (replacing the current `-32601`), validate URIs (`file://`, path-traversal #372), and fire `notify_roots_list_changed` on cwd change. Rows 363/364/367/371-374/378/380/381 flip from N/A → ✅ on implementation.
