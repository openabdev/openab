...
| 274 | Ping frequency SHOULD be configurable | SHOULD | ❌ | no ping config field on `ServerConfig::Stdio` / `ServerConfig::Http` (`config.rs:21-36`); `McpConfig` itself at `config.rs:13-16` is just a server-list wrapper |
