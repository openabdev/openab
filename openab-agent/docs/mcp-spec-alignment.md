...
**Section-level finding**: openab-agent does **not** advertise the `sampling` (or `sampling.tools`) client capability and does not implement a sampling surface. The runtime initialises rmcp with the named `OpenabClientHandler` (`src/mcp/runtime.rs`); its `get_info()` override advertises only `capabilities.roots`, leaving the `sampling` field unset (it mutates a `ClientInfo::default()`), so `create_message` inherits the SDK default that returns `Err(-32601)` method-not-found (SDK `handler/client.rs:92-100`). Grep across `src/` for `sampling` / `CreateMessage` / `create_message` returns zero functional hits. Conversation turns are produced directly by the LLM provider layer (`src/llm.rs` — `AnthropicProvider` / `OpenAiProvider`), so the value proposition of sampling (server borrows the client's LLM) would route back to our own provider and has no demand signal today. Most rows below are therefore N/A by deliberate abstention; rows 387a / 405 are ⚠️ (vacuous) because the blanket default rejects the whole request rather than the specific field the row describes — acceptable since we never claim sampling support. See Section 17 §3 (Trust / Safety) for the consent obligation that would attach if sampling is ever enabled.
