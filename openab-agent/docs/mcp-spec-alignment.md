...
| 18 | Implementations MUST support JSON Schema 2020-12 for schemas without explicit `$schema` | MUST | ✅ | `tool_summary::unsupported_schema_dialect` (`src/mcp/meta_tool.rs`) treats an absent `$schema` as the implied 2020-12 default — passes through unflagged (the common server case) |
| 19 | Implementations MUST validate schemas according to declared/default dialect | MUST | ⚠️ | deliberate non-validation (option (b), §0 plan): rmcp 1.7.0 ships no validator (`Cargo.lock` has neither `jsonschema` nor `valico`) and we relay `inputSchema` straight to the LLM, which tolerates any dialect — so a validator would be compliance theatre. Instead of silent passthrough we *detect* a foreign declared dialect and surface it (row 20) |
| 20 | Implementations MUST handle unsupported dialects gracefully (return error indicating unsupported) | MUST | ✅ | `tool_summary` flags any tool whose `inputSchema.$schema` declares a non-2020-12 dialect with `available: false` + `unavailable_reason` (advisory NeedsAttention; not a hard call-refusal since we never validate `arguments` against the schema). Test `tool_summary_flags_unsupported_schema_dialect` |
| 21 | Implementations SHOULD document which schema dialects they support | SHOULD | ✅ | documented here: openab-agent supports JSON Schema **draft 2020-12** (the MCP-mandated dialect); other dialects are relayed but surfaced as NeedsAttention (§0 plan, rows 18-20) |
...
- [x] **Rows 18-21 (JSON Schema 2020-12)**: DONE via **option (b)** (the recommended path — a validator for schemas we merely relay is compliance theatre). Decision recorded: openab-agent's supported tool-`inputSchema` dialect is **draft 2020-12**; rmcp 1.7.0 ships no dialect validator (`Cargo.lock` has neither `jsonschema` nor `valico`, only `schemars` for gen + `serde`), and we pass `inputSchema` straight to the LLM, so we do not add one. An absent `$schema` is treated as the implied 2020-12 default and passes through unflagged (row 18). A declared non-2020-12 dialect is surfaced (row 20, see below) rather than silently relayed (row 19's graceful-handling intent). Supported-dialect statement documented here (row 21).
  - **Eval**: openab-agent layer · docs decision + small detection helper · **fit: in-scope, landed (option b)**.
- [x] **Rows 19-20**: DONE — `tool_summary` (`src/mcp/meta_tool.rs`) calls `unsupported_schema_dialect(&t.input_schema)` (scheme/fragment-insensitive match against `json-schema.org/draft/2020-12/schema`); a foreign declared dialect adds `available: false` + `unavailable_reason: "input_schema declares JSON Schema dialect \"…\"; openab-agent supports only draft 2020-12"`. Reuses the tick #18 `available`/`unavailable_reason` mechanism and is **additive** with the `taskSupport=required` reason (a tool can carry both, joined by `; `). Surfaced through both `list_tools` and `describe_tool`. Advisory only — NOT a hard `call` refusal, because we never validate `arguments` against the schema, so a working tool whose author pinned an older dialect stays callable. Unit test `tool_summary_flags_unsupported_schema_dialect`.
  - **Eval**: openab-agent only · drop-in (~30 LOC + helper + test) · **fit: in-scope, landed**. Surfacing "we don't know this dialect" is the bare-minimum graceful handling the MUST asks for.
...
- [x] **Documentation**: README MCP section should explicitly call out (a) JSON Schema dialect support / passthrough behavior, (b) that `_meta` keys are passed through opaquely, (c) icon-rendering is N/A (we're not a UI client). **DONE** — written in [`docs/mcp.md`](./mcp.md) §"Tools and schemas" (dialect passthrough + advisory non-validation, `_meta` opaque passthrough, icons N/A) + the doc-wide capability matrix + the vendor-comparison matrix.
...
- [x] **Rows 220, 222, 223 (PKCE methods verification)**: once AS-metadata discovery lands, check `code_challenge_methods_supported` contains `S256` before issuing the request; abort with clear error if absent. Until discovery is done, document the "always send PKCE, trust the AS" behaviour as a known soft-violation.
  ...
  - **DONE (this batch): closed by the login-path swap above.** With discovery now live, `configure_client` rejects an AS whose metadata lacks S256 in `code_challenge_methods_supported`, and `get_authorization_url` generates an S256 challenge unconditionally (rmcp `auth.rs:988`). No separate openab-agent check needed. Spec-table row ✅ flips for 220/222/223 deferred to the closeout WITH Mira.
...
- [x] **Document the abstention explicitly** — DONE (doc): added a Section 9 section-level finding (above) stating openab-agent advertises no `sampling` / `sampling.tools` capability — `OpenabClientHandler::get_info()` advertises only `capabilities.roots`, leaving the `sampling` field unset, so the default `create_message` returns `-32601`. Cross-references `src/llm.rs` (native turn construction) and Section 17 §3 (consent obligation if ever enabled). The optional separate `docs/mcp-client-capabilities.md` / inline `mod.rs` comment is deferred — the canonical alignment-doc finding makes the abstention discoverable.
...
- [~] **Row 501 — validate structured results against `outputSchema`** when both are present; emit warning + downgrade to text on mismatch. **(KNOWN GAP — see DECISION below; conformance gate is server-side.)**
  ...
  - **DECISION (Brett 2026-06-05): known gap — not implemented; the conformance gate is server-side.** Per the 2025-11-25 tools spec the **server MUST** emit `structuredContent` that conforms to the declared `outputSchema`, while the **client only SHOULD** validate it. So the authoritative validation gate lives on the server (it is both producer *and* obligated to conform); client-side re-validation is redundant defensive coverage against a misbehaving server — the same family as arg-side validation theatre (in both directions the normative gate is server-side, not us). Combined with the cost — openab-agent carries no JSON Schema validator (lean dep tree, not even `regex`), so this SHOULD-level behaviour would mean a new dependency (`boon` lean / `jsonschema` heavy) — the CP value is too low to justify now. `structuredContent` is relayed to the LLM as-is (server trusted as producer). Documented as a known gap in `mcp.md` §8; revisit + adopt `boon` only if a real server emits schema-divergent output.
...
- [x] **§3. Document the sampling abstention (rows 619-620)** — DONE (doc): the Section-level disposition (above) now explains openab-agent advertises no `ClientCapabilities { sampling: .. }` (`OpenabClientHandler::get_info()` advertises only `capabilities.roots`, leaving `sampling` unset; default `create_message` returns `Err(-32601)`), and that a future Host wanting sampling must (a) advertise the capability and (b) override `create_message` to gate through ACP user-approval before forwarding to `src/llm.rs`. Cross-references the Section 9 (Client / Sampling) section-level finding added this pass.
...
- [x] **§7. Sampling capability gating tracker** — DONE (tracking note): records that when (if) Section 9 ships a sampling client surface, rows 619 / 620 flip from N/A-vacuous to requiring explicit user approval. Today's N/A is correct (no capability advertised — see §3 + Section 9 finding); this tracker prevents accidentally enabling sampling without a consent flow. No code change today.
