...
| # | Item | Normative | Status | Location / Notes |
|---|---|---|---|---|
| 414 | Clients supporting elicitation MUST declare `elicitation` capability | MUST | ⏳ | **In-scope (Brett 2026-06-05): form mode.** Currently no `ClientCapabilities.elicitation`; lands with the HostBridge keystone — will declare `elicitation.form` (schema_validation on), `url: None`. Gating row for everything below — see Improvement Plan §1 DECISION |
...
| 439 | Clients MUST return `-32602` when elicitation mode not declared in capabilities | MUST | ✅ | `OpenabClientHandler::create_elicitation` (`src/mcp/runtime.rs`) returns `Err(ErrorData::invalid_params("elicitation capability not declared", None))` (= -32602) instead of the SDK default `Ok(Decline)`. We declare no `elicitation` capability, so any inbound `elicitation/create` is now rejected as `INVALID_PARAMS` per spec. Closed by Improvement Plan §2 |
